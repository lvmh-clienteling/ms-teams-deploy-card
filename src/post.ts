...
try {
  ...
  setTimeout(async () => {
    ...
    if ((showCardOnExit && !showCardOnFailure) || (showCardOnFailure && workflowRunStatus?.conclusion !== 'success')) {
      await formatAndNotify('exit', workflowRunStatus?.conclusion ?? 'in_progress', workflowRunStatus?.elapsedSeconds);
    } else {
